...
{
    public partial class frm_FoodManager : Form
    {
        public event EventHandler ProductAdded;
        private int check = 0;

        public frm_FoodManager()
        {
            InitializeComponent();
        }
        ...
        public void btnBack_Click(object sender, EventArgs e)
        {
            ...
            // Yêu cầu frm_Main đưa frm_Order lên trước
            if (Forms.frm_Main.Instance != null)
            {
                foreach (Form child in Forms.frm_Main.Instance.MdiChildren)
                {
                    if (child is frm_Order)
                    {
                        child.BringToFront();
                        child.Activate();
                        break;
        ...
        private void frm_FoodManager_Load(object sender, EventArgs e)
        {
