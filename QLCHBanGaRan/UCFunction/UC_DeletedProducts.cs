...
namespace QLCHBanGaRan.UCFunction
{
    public partial class UC_DeletedProducts : UserControl
    {
        ...
        private void btnRestore_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Vui lòng chọn sản phẩm để khôi phục!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            try
            {
                string maSP = dtDeletedProducts.SelectedRows[0].Cells["MaSP"].Value.ToString();
                ...
                {
                    MessageBox.Show("Đã khôi phục sản phẩm!", "Thành công", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    LoadDeletedProducts();
                }
                else
                {
                    MessageBox.Show("Không thể khôi phục sản phẩm!", "Thất bại", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            catch (Exception ex)
            {
        ...
        private void btnDeletePermanently_Click(object sender, EventArgs e)
        {
            if (dtDeletedProducts.SelectedRows.Count == 0)
            {
                MessageBox.Show("Vui lòng chọn sản phẩm để xóa vĩnh viễn!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            try
            {
                ...
                string tableName = cbCategory.SelectedItem.ToString() == "Đồ ăn" ? "DoAn" : "DoUong";
                string idColumn = tableName == "DoAn" ? "MaMon" : "MaDoUong";
                if (MessageBox.Show("Bạn có chắc muốn xóa vĩnh viễn sản phẩm này?", "Xác nhận", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    {
                        MessageBox.Show("Không thể xóa vĩnh viễn sản phẩm!", "Thất bại", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }
            }
            catch (Exception ex)
